...
public class DebtDashboardService implements DebtDashboardControllerInterface {
    ...
    @Override
    public ResponseEntity<List<AdequacyFullReport>> getAdequacyGeneralReport(Map<String, String> header, GeneralReportRequest reportRequest){
        List<Announcements> allAnnouncements = this.announcementRepo.findAllByAnnouncementStatusIsNotIn(Constants.CONDITIONS);
        List<AdequacyFullReport> allReports = new ArrayList<>();

        for(Announcements ann : allAnnouncements) {
            List<SubSubject> subSubjects = this.announcementRepo.findAllFieldValue(ann.getAnnId());
            List<ParametersMaster> parameters = this.announcementRepo.findAllParameter(ann.getAnnId());
            List<AdequacySubParameter> adequacySubParameters = new ArrayList<>();
            for(ParametersMaster param : parameters) {
                List<String> subParameters = this.subParameterRepo.findAllByParameterMasterIn(ann.getAnnId(), param.getId());
                AdequacySubParameter parameterSubParameter = AdequacySubParameter.builder()
                        .parameterName(param.getParameterName())
                        .subParameters(subParameters)
                        .build();
                adequacySubParameters.add(parameterSubParameter);
            }
            List<Communications> clarificationList = this.communicationRepo.findAllByAnnouncement(ann);
            List<AdequacyClarification> adequacyClarifications = new ArrayList<>();
            for(Communications comm : clarificationList) {
                Long noOfResponse = 1L;
                AdequacyClarification clarification = AdequacyClarification.builder()
                        .clarification(true)
                        .clarificationBody(comm.getEmailBody())
                        .clarificationDt(comm.getCreatedDt())
                        .clarificationResponseDt(comm.getCommunicationResponse() == null ? null : comm.getCommunicationResponse().getCreatedDt())
                        .clarificationTo(this.getAllEmailTo(comm.getEmailTo()))
                        .noOfResponse(noOfResponse)
                        .build();
                adequacyClarifications.add(clarification);
            }

            List<AdequacyInternal> adequacyInternals = new ArrayList<>();
            List<Communications> internalList = this.communicationRepo.findCommunicationsByAnnouncementAndEmailType(ann, Constants.INTERNAL);
            for(Communications comm : internalList) {
                AdequacyInternal internal = AdequacyInternal.builder()
                        .internal(true)
                        .internalBody(comm.getEmailBody())
                        .internalDt(comm.getCreatedDt())
                        .internalTo(this.getAllEmailTo(comm.getEmailTo()))
                        .build();
                adequacyInternals.add(internal);
            }

            List<OcrKeywordResponseBody> genericKeywords = this.ocrAttachmentRepo.findAllByAnnouncement(ann.getAnnId(), "generic");
            List<OcrKeywordResponseBody> subjectWise = this.ocrAttachmentRepo.findAllByAnnouncement(ann.getAnnId(), "sensitive");
            Attachments doc = this.attachmentRepo.findByAnnouncementAndStatus(ann, Constants.ANNOUNCEMENT_ATTACHMENT);

            DocUploadDetails docUploadDetails = DocUploadDetails.builder()
                    .fileName(doc.getFileName())
                    .fileType(doc.getFileType())
                    .fileUuid(doc.getId())
                    .build();

            AdequacyMisc misc = AdequacyMisc.builder()
                    .genericKeywords(genericKeywords)
                    .subjectWise(subjectWise)
                    .attachedFile(docUploadDetails)
                    .build();

            AdequacyFullReport report = AdequacyFullReport.builder()
                    .adequacyDt(ann.getAdequacyDt())
                    .adequacyType(ann.getAdequacyType())
                    .announcementRemark(ann.getRemarks())
                    .applicationNo(ann.getAnnId())
                    .applicationStatus(ann.getAnnouncementStatus())
                    .broadcastDt(ann.getBroadcastDt())
                    .categoryName(ann.getSubjectTypeId().getCategory().getCategoryName())
                    .companyName(ann.getCompanyName())
                    .exchangeRemarks(ann.getException() == null ? null : ann.getException().getRemark())
                    .handledBy(ann.getHandledBy())
                    .subjectName(ann.getSubjectTypeId().getSubjectName())
                    .subSubjectList(subSubjects)
                    .verifiedBy(ann.getVerifiedBy())
                    .verifiedDt(ann.getVerifiedDt())
                    .clarifications(adequacyClarifications)
                    .internals(adequacyInternals)
                    .miscs(misc)
                    .nonCompliance(null)
                    .parameters(adequacySubParameters)
                    .softMails(null)
                    .wrongSubjects(null)
                    .build();
    ...
    public ResponseEntity<List<AnnouncementFullReport>> getAnnouncementReport(GeneralReportRequest reportRequest, Map<String, String> header) {
        ...
        for(Announcements ann : announcements) {
            ...
            List<SubSubject> allSubSubject = this.announcementRepo.findAllFieldValue(ann.getAnnId());
            AnnouncementFullReport report = AnnouncementFullReport.builder()
                    .applicationNo(ann.getAnnId())
                    .companyName(ann.getCompanyName())
                    .categoryName(ann.getSubjectTypeId().getCategory().getCategoryName())
                    .subjectName(ann.getSubjectTypeId().getSubjectName())
                    .subSubjectType(allSubSubject)
                    .announcementText(ann.getAnnouncementText())
